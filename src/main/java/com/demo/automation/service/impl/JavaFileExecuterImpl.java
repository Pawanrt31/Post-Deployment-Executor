...
@Component
@Slf4j
public class JavaFileExecuterImpl implements Executer {
	...
	public void automateExecution(String name) {
		List<File> files = automationUtils.getFiles(
				name + AutomationConstants.PATH_SEPARATOR + AutomationConstants.JAVA_FOLDER,
				AutomationConstants.JAR_EXTENSION);
		...
		try {
			List<String> file1 = automationUtils.getListFromJSON(AutomationConstants.JAVA_FOLDER);
			file1.stream().forEach(name2 -> {
				log.info("---------JAVA JAR EXECUTION-------------");
				log.info("Executing "+ name2);
				automationUtils.executeFile(AutomationConstants.JAVA_FOLDER + AutomationConstants.JAR_EXECUTE
						+ config.getFolderRelativePath() + AutomationConstants.PATH_SEPARATOR
						+ AutomationConstants.JAVA_FOLDER + AutomationConstants.PATH_SEPARATOR + name2);
			});
			if ((fileNames.size() - file1.size()) != 0) {
				...
				remainingFiles.stream().forEach(nameFile -> {
					...
					automationUtils.executeFile(AutomationConstants.JAVA_FOLDER + AutomationConstants.JAR_EXECUTE
							+ config.getFolderRelativePath() + AutomationConstants.PATH_SEPARATOR
							+ AutomationConstants.JAVA_FOLDER + AutomationConstants.PATH_SEPARATOR + nameFile);
				});
			}
		} catch (Exception e) {
			e.printStackTrace();
